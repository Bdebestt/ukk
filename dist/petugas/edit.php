...
session_start();
    if (isset($_POST['edit_petugas'])) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $nipp=input($_POST["nipp"]);
            $jkk=input($_POST["jkk"]);
            $emaill=input($_POST["emaill"]);
            $no_telpp=input($_POST["no_telpp"]);
            $alamatt=input($_POST["alamatt"]);
            $statuss=input($_POST["statuss"]);
...
<form action="petugas/edit.php" method="post" enctype="multipart/form-data">
    <div class="row">
        ...
        <div class="col-sm-5">
            <div class="form-group">
                ...
                <input type="text"  class="form-control"  value="<?php echo $data['nipp']; ?>" placeholder="Masukan Nomor Induk Pegawai" disabled required>
                <input type="hidden" name="nipp" class="form-control"  value="<?php echo $data['nipp']; ?>"  >
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-sm-4">
            <div class="form-group">
                <label>Email:</label>
                <input type="email" name="email" class="form-control" value="<?php echo $data['emaill'];?>" placeholder="Masukan Email" required>
            </div>
        </div>
        <div class="col-sm-3">
            <div class="form-group">
                <label>No Telp:</label>
                <input type="text" name="no_telp" class="form-control" value="<?php echo $data['no_telpp'];?>" placeholder="Masukan No Telp" required>
            </div>
        </div>
        <div class="col-sm-5">
            <div class="form-group">
                ...
                <div class="form-check-inline">
                    <label class="form-check-label">
                        <input type="radio" <?php if (isset($data['jkk']) && $data['jkk']==1) echo "checked"; ?> class="form-check-input" name="jk" value="1" required>Laki-laki
                    </label>
                    <label class="form-check-label">
                        <input type="radio" <?php if (isset($data['jkk']) && $data['jkk']==2) echo "checked"; ?> class="form-check-input" name="jk" value="2" required>Perempuan
                    </label>
                </div>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-sm-7">
            <div class="form-group">
                <label>Alamat:</label>
                <textarea class="form-control" name="alamat" rows="4" id="alamat"><?php echo $data['alamatt'];?></textarea>
            </div>
        </div>
    </div>
    <div class="row">
    ...
    <div class="row">
        <div class="col-sm-3">
        <label>Foto:</label><br>
            <img src="karyawan/foto/<?php echo $data['fotoo'];?>" id="preview" width="90%" class="rounded" alt="Cinque Terre">
            <input type="hidden" name="foto_saat_ini" value="<?php echo $data['fotoo'];?>" class="form-control" />
        </div>
        <div class="col-sm-4">
            <div id="msg"></div>
